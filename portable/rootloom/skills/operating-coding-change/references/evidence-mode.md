...
In the native Codex package, deterministic helpers live under
`plugins/rootloom/resources/evidence/`. Existing low-level CLIs and frozen evidence
formats remain compatible.

Before promising Evidence Mode, resolve the complete plugin root and verify that
`resources/evidence/analyze_change.py`, `begin_review.py`, `seal_contract.py`,
`orchestrate_evidence.py`, and `finalize_change.py` are present. A Skills-only package
may intentionally omit those helpers. If any required helper is unavailable, stop and
report Evidence Mode as unavailable in the installed package; never substitute an
ordinary test run or a reconstructed baseline and call it Rootloom Evidence.
